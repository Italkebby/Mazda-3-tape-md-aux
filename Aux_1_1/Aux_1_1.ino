...
void isr(){
  //on pin bus change
  if(!digitalRead(DATA_PIN)){  //if bus low
    lowTime = micros();        //store time when bus went low
    if((micros()-highTime) > 7500){    //clear everything if its been over 7.5ms since last pulse
     buff = 0;
     buffPos = 0; 
    }
  }
  else{    //if bus high
    if(abs((int)(micros()-lowTime) - 1700) < 300){    //if bus was low for about 1.7 ms
